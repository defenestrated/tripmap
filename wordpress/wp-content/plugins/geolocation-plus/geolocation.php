<?php
/*
Plugin Name: Geolocation Plus
Plugin URI: http://wordpress.org/extend/plugins/geolocation-plus/
Description: Displays post geotag information on an embedded map & generates a GeoRSS feed.
Version: 0.1.5.1
Author: Adrian Short
Author URI: http://adrianshort.org/
License: GPL2
*/
...
function display_location($content)  {
  ...
  if((!empty($latitude)) && (!empty($longitude) && ($public == true) && ($on == true))) {
    if (!empty($address)) {
      $link_text = 'Posted from ' . esc_html($address) . '.';
    } else {
      $link_text = 'Posted from here.';
    }
		$html = '<a class="geolocation-link" href="#" id="geolocation'.$post->ID.'" name="'.$latitude.','.$longitude.'" onclick="return false;">' . $link_text .'</a>';
...
function reverse_geocode($latitude, $longitude) {
    ...
    if ($iserr != 1) {
            ...
            foreach ($json->results as $single_result)
                {
                    foreach($single_result->address_components as $addressPart) {
...
function add_settings() {
	if ( is_admin() ){ // admin actions
		add_options_page('Geolocation Plus Plugin Settings', 'Geolocation Plus', 'administrator', 'geolocation.php', 'geolocation_settings_page', __FILE__);
...
/* Add GeoRSS feed autodiscovery link to page <head> */
function geolocation_plus_georss_feed() {
  ?>
    <link rel="alternate" type="application/rss+xml" title="<?php echo get_bloginfo('name') ?> &raquo; GeoRSS Feed" href="<?php echo plugins_url('georss.php', __FILE__) ?>" />
  <?php
}
